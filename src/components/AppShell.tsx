...
export function AppShell({ user }: Props) {
  ...
  const creatingRef = useRef<{ projectId: string; promise: Promise<string | null> } | null>(null);
  ...
  /** Hard-cut to `id` (or none). Bumps gen so in-flight loads cannot clobber. */
  ...
  const switchTo = useCallback(
    async (id: string, snapshot?: SessionDetail | null) => {
      const gen = selectSession(id, snapshot);
    ...
    (sid: string) => {
      ...
      const handleEvent = (ev: MessageEvent) => {
        ...
        if (event.type === "message") {
          ...
          setDetail((prev) => (prev && prev.id === sid ? { ...prev, status: "idle" } : prev));
          setSessions((list) =>
            list.map((row) => (row.id === sid ? { ...row, status: "idle" } : row)),
          );
          void settleTurn();
        }
      };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (selectGen.current !== 0) return;
        const stored = readStoredSelectedId();
        const pick =
          stored && list.some((s) => s.id === stored) ? stored : list[0]?.id;
        if (pick) {
          if (selectGen.current !== 0) return;
          const gen = selectSession(pick);
          await loadSession(pick, { gen });
        }
      } catch (err) {
        if (!cancelled) {
  ...
  }, [refreshLists, loadSession, selectSession]);
  ...
  async function createSession(projectId: string): Promise<string | null> {
    if (creatingRef.current?.projectId === projectId) return creatingRef.current.promise;
    if (creatingRef.current) {
      try {
        await creatingRef.current.promise;
      } catch {
        // prior create failed; start this click
      }
      if (creatingRef.current?.projectId === projectId) return creatingRef.current.promise;
    }
    ...
    creatingRef.current = { projectId, promise: pending };
    void (async () => {
      try {
        ...
        await switchTo(id, data.session);
        await refreshLists();
        ...
        if (creatingRef.current?.promise === pending) creatingRef.current = null;
      }
    })();
    return pending;
  }
  ...
  async function rename(id: string, title: string) {
    ...
    await refreshLists();
    if (!selectionStale(gen, id) && selectedRef.current === id) {
      await loadSession(id, { reconcile: false, gen });
    }
    if (!selectionStale(gen)) setNotice(`Renamed to ${title}`);
  }
  ...
  async function patchPrefs(prefs: { approval?: string; model?: string; variant?: string }) {
    ...
    try {
      ...
      if (!selectionStale(gen)) setNotice(err instanceof Error ? err.message : "update failed");
    }
  }
  ...
  async function sendPrompt(sid: string, text: string, fromQueue = false) {
    if (!sid) return;
    if (!fromQueue && selectedRef.current !== sid) return;
    const gen = selectGen.current;
    ...
    try {
      ...
      const paint = !selectionStale(gen, sid);
      if (status === 202) {
        waitForStream = true;
        ...
        await refreshLists();
        if (titled && !selectionStale(gen, sid)) {
  ...
  async function addShare(username: string, shareRole: ShareRole) {
    ...
    const gen = selectGen.current;
    if (!sid) return;
    await api(`/api/sessions/${sid}/shares`, {
      method: "POST",
      body: JSON.stringify({ username, role: shareRole }),
    });
    if (!selectionStale(gen, sid)) await loadSession(sid, { reconcile: false, gen });
    await refreshLists();
  }
    ...
    switch (cmd) {
      ...
      case "rename":
        if (!accountWrite || !can(role, "write")) {
          ...
          await rename(selectedRef.current, arg);
        }
        break;
      case "rewind":
        if (!accountWrite || !can(role, "write") || !selectedRef.current) setNotice("Read-only.");
        else await runAction(selectedRef.current, "rewind");
        break;
      case "compact":
        if (!accountWrite || !can(role, "write") || !selectedRef.current) setNotice("Read-only.");
        else await runAction(selectedRef.current, "compact");
        break;
      case "find":
        if (selectedRef.current) {
          mutateSlot(selectedRef.current, (s) => {
  ...
  async function runShell(command: string) {
    ...
    try {
      const data = await api<{ session: SessionDetail }>(`/api/sessions/${sid}/shell`, {
        method: "POST",
        body: JSON.stringify({ command }),
      });
      if (!selectionStale(gen, sid)) setDetail(data.session);
    } catch (err) {
      if (!selectionStale(gen)) setNotice(err instanceof Error ? err.message : "shell failed");
    }
  }
      ...
      {loadError ? (
        ...
        <div className={"panes" + (collapsed ? " artifacts-collapsed" : "")}>
          <Sidebar
            ...
            onShare={(id) => {
              void switchTo(id).then(() => setShareOpen(true));
            }}
